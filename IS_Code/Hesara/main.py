...
Demonstrates building a Graph from hardcoded sample nodes and from a
NavMesh JSON file, then prints the adjacency list.

Run:
    python main.py
    python main.py sample_navmesh.json
"""
...
from graph_utils import euclidean_distance, export_graph_to_json
...
def main():
    ...
    if len(sys.argv) > 1:
        navmesh_file = sys.argv[1]
        print(f"\n[1] Loading NavMesh from {navmesh_file}...")
        extractor = GraphExtractor(proximity_threshold=200.0)
        graph = extractor.extract_from_file(navmesh_file)
        print(f"    Loaded {graph.node_count()} nodes from JSON.")
    else:
